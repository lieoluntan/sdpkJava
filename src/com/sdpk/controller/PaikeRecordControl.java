...
public class PaikeRecordControl extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (qqiu.equals("test") || qqiu.equals("add") || qqiu.equals("delete")
				...
				|| qqiu.equals("queryConflict") || qqiu.equals("listcla")
				|| qqiu.equals("listKeDate")) {
			// 2 将前台json数据转成实体对象
			...
			// end前台数据转换
			ArrayList<PaikeRecord> resultList = paikeRecordService
					.selectConflict_batch(pr_List);
			backResult.setMessage("信息值：成功");
			backResult.setQingqiu("list查询列表");
			backResult.setData(resultList);
			...
			try {
				// 判断pkType格式是否为LineUpA，LineDownB字符串
				String pktype = prp.getPkType();
				if (pktype.equals("LineUpA") || pktype.equals("LineDownB")) {
					if (pktype.equals("LineUpA")) {
						prp.setPkTypeName("线上");
					}
					if (pktype.equals("LineDownB")) {
						prp.setPkTypeName("线下");
					}
					System.out.println(prp.getPkType());
	...
	private PaikeRecordPre MapToPaikeRecordPre(Map<String, Object> map) {
		...
		PaikeRecordPre prp = new PaikeRecordPre(id, claUuid, courseUuid,
				empUuid, classroomUuid, keDateTime, keStartTime, keLongTime,
				keCount, status, weekDay, empConflict, croomConflict);
		prp.setPkType(pkType);
		return prp;
	...
	private void qqiuChoice(String qqiu, PaikeRecord paikeRecord) {
		...
		if (listcla) {
			ArrayList<PaikeRecord> resultList = paikeRecordService
					.getListByclaUuid(paikeRecord.getClaUuid());
			backResult.setMessage("信息值：成功");
			backResult.setQingqiu("listcla查询列表班级排课记录");
			backResult.setData(resultList);
		}// end if listcla
		if (listKeDate) {
			ArrayList<PaikeRecordView> resultList = paikeRecordService
					.getListByKeDate(paikeRecord.getKeDateTime());
			backResult.setMessage("信息值：成功");
