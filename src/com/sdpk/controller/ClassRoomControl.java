...
public class ClassRoomControl extends HttpServlet {
  ...
  Logger logger=Logger.getLogger(ClassRoomControl.class);
...
//    delete = qqiu.equals("delete");
    ...
    if (test) {
    	logger.error("test,log4j日志测试成功,ClassRoomControl");
    ...
    if (add) {
      ...
      backResult.setMessage(result=="yes" ? "(已存在重复名字)" +classRoom.getName() : "新增成功" );
    ...
    if (on_off) {
    	...
    	if(oAc.equals("open")||oAc.equals("close")){
    		...
    		if(oAc.equals("close")){flagQqiu = "off";}
    		 result = classRoomService.getonoff(classRoom);
