...
public class PaikeRecordServiceImpl implements PaikeRecordService {
	...
	public String insert(PaikeRecord paikeRecord) {
		...
		System.out.println("^^在PaikeRecordServiceImpl收到数据 ："
				+ paikeRecord.toString() + "收到结束!");
...
//			String priceType = cour.getCategory();
		...
		System.out.println(claUuid);
		ArrayList<PaikeRecord> db_List = paikeRecordDao
		...
		int sumcountAZong = 0;
		int sumcountBZong = 0;
		ArrayList<Class_Contract> cc_List = class_ContractDao
				.getListBycla(claUuid);
		for (Class_Contract one : cc_List) {
			String contrUuid = one.getContrUuid();
			Contrtext contr = contrtextDao.getOne(contrUuid);
			int sumcountA = Integer.valueOf(contr.getSumLineUpA()).intValue();
			int sumcountB = Integer.valueOf(contr.getSumLineDownB()).intValue();
	...
	public String delete(String uuid) {
		...
		if (uuid != null && uuid != "") {
			...
			System.out
					.println("PaikeRecordServiceImpl delete方法中的uuid为空，或格式不正确，请联系管理员");
			return msg;
		}
...
//				System.out.println(count1200db);
	...
	public ArrayList<PaikeRecord> getListByclaUuid(String claUuid) {
		// TODO Auto-generated method stub
		ArrayList<PaikeRecord> paikeRecordList = paikeRecordDao
				.getListByclaUuid(claUuid);
	...
	public PaikeRecord getByUuid(String uuid) {
		...
		if (uuid != null && uuid != "") {
			...
			System.out
					.println("PaikeRecordServiceImpl getByUuid方法中的uuid为空，或格式不正确，请联系管理员");
	...
	@Override
	public PaikeRecord selectConflict(PaikeRecord paikeRecord)
			throws ParseException {
		System.out.println(paikeRecord.getPkType());
		// TODO Auto-generated method stub
		// 初始化0.0
		...
		try {
			System.out.println(df.parse(paikeRecord.getKeStartTime())
					+ "前台传的时分秒格式正确");
		} catch (Exception e) {
	...
	public boolean flagConflict(long pai_startTime, long pai_longTime,
			ArrayList<PaikeRecord> PRList) throws ParseException {
	...
	@Override
	public ArrayList<PaikeRecord> selectConflict_batch(
			ArrayList<PaikeRecord> PR_List) {
		// TODO Auto-generated method stub
	...
	@Override
	public ArrayList<PaikeRecord> getPaikePre(PaikeRecordPre paikeRecordPre)
			throws ParseException {
		// TODO Auto-generated method stub
		...
		ArrayList<Date> paiDayList = getPaikePre_DateList(paikeRecordPre,
				yearAllDayList);
		// 步骤三:根据 步二日子列表获得排课记录表（年月日）
		ArrayList<PaikeRecordPre> reList = new ArrayList<PaikeRecordPre>();
		for (Date aDay : paiDayList) {
			PaikeRecordPre copyOne = new PaikeRecordPre("预览id",
					paikeRecordPre.getClaUuid(),
					paikeRecordPre.getCourseUuid(),
					paikeRecordPre.getEmpUuid(),
					paikeRecordPre.getClassroomUuid(),
					paikeRecordPre.getKeDateTime(),
					paikeRecordPre.getKeStartTime(),
					paikeRecordPre.getKeLongTime(),
					paikeRecordPre.getKeCount(), paikeRecordPre.getStatus(),
					paikeRecordPre.getWeekDay(),
					paikeRecordPre.isEmpConflict(),
					paikeRecordPre.isCroomConflict()
					);
			copyOne.setPkType(paikeRecordPre.getPkType());
			copyOne.setPkTypeName(paikeRecordPre.getPkTypeName());//构造函数没存，单独存上课类型（线上或线下）
		...
		for (PaikeRecord one : resultList) {
			...
			ClassRoom croom = classRoomDao.getByUuid(crUuid);
			String courName = cour.getName();
//			String cageName = cour.getCategory();新版接口，不用Category，改用pkType
			String empName = emp.getName();
			String croomName = croom.getName();
...
//			one.setCategoryName(cageName);新版接口，不用Category，改用pkType
    ...
    **/
	 ...
	 * @return 用来获取对应次数的日期列表数据
	 */
	public ArrayList<Date> getPaikePre_DateList(PaikeRecordPre paikeRecordPre,
			ArrayList<Date> yearAllDayList) {
				...
				if (aDay.after(startDay))
					...
					if (nowSize < count) {
						...
						if (six) {
							if (week.equals("星期六")) {
								paiDayList.add(aDay);
								System.out.println("添加的次数：" + nowSize + "查看星期："
										+ aDay);
							}
						}
						if (seven) {
	...
	public ArrayList<PaikeRecordView> getListByKeDate(String keDateTime) {
		// TODO Auto-generated method stub
		ArrayList<PaikeRecordView> prvList = paikeRecordDao
				.getListByKeDate(keDateTime);
