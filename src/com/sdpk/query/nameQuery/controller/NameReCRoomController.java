...
import com.sdpk.query.nameQuery.service.impl.NameReCRoomServiceImpl;
import com.sdpk.utility.T_DataControl;
import com.sdpk.utility.T_DataMap2Bean;

public class NameReCRoomController extends HttpServlet{

	private NameReCRoomService nameReClaService=new NameReCRoomServiceImpl();
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (qqiu.equals("query")) {
			...
			if (str != null && str != "" && str.length() != 0) { // 非空判断，防止前台传空报500服务器错误中的空指针
				...
				cla = t_map2bean.MapToClassRoom(map);
			} else {
	...
	private void qqiuChoice(String qqiu, ClassRoom cR) {
		...
		if (test) {
			String flag = nameReClaService.getClassRoomByName(cR);// 得到名字是否已存在
			String flag1 = nameReClaService.getClaSSRoomByName1(cR);// 得到yes/no
